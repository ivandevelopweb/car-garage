...
export function Brand({ compact = false, light = false }: BrandProps) {
  return (
    <Link className={`brand ${light ? 'brand--light' : ''}`} to="/" aria-label="Автозвук, Тонування — головна">
      ...
      {!compact && <span className="brand__words"><strong>Автозвук,</strong><span>Тонування</span></span>}
    </Link>
  )
}
